...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Material Invoice Bill</title>
        <style>
        h2 
        { 
            display: flex; 
    ...
    <body class="container" style="font-family: Times New Roman; font-size: 16px;" 
        ...
        <table class="table" style="margin-top: 7%">
            ...
            $dislpaydata = "SELECT invoice.*, purchase_order.Item_Code, item_master.Item_Name "
                        ...
                        . "AND Invoice_Date Between '$fdt' and '$tdt' order by Invoice_Date";
            $result = mysqli_query($conn, $dislpaydata);
            if(mysqli_num_rows($result)>0)
            {
                while ($row = mysqli_fetch_assoc($result)) 
                {
                    $total_bill_amount += $row["Total_Pay_Amount"];
            ?>
            <tr>
          ...
          <?php
           ...
           <tr>
               ...
               <td><b><?php echo $total_bill_amount;?></b></td>
           </tr>
        </table>
        <br>
    </body>
